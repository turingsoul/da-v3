...
export default class handle extends WidgetBase {
        ...
        switch (value) {
            case "0":
                ...
                formatterFn = function(data) {
                    ...
                    num =
                        query && dataConfig
                            ? Util.formatData2Str(num, dataConfig)
                            : num;
    ...
    _handleDataSetData(drillName) {
        ...
        zipDataFilter.length &&
            zipDataFilter[0].forEach((e, i) => {
                let formatData = Util.formatData2Num(e, dataConfig);
                dataObj.data.push({
        ...
        option.tooltip.formatter = e => {
            var content = "";
            e.data.value = Util.formatData2Str(e.data.value, dataConfig);
    ...
    _getChangeData(data, ownOption) {
        var array = [];
        const { types, counter } = data;
        var type;
        //判断是否开启下转功能
        var getIfNull = ownOption.clickEvent !== "drill";
        //未开启下转
        if (getIfNull) {
            type = this.getType(types, types[0].length);
            array = counter.map(e => e[0]);
        } else {
            type = this.getType1(types, types[0].length);
            //处理数据为连接数据
            let selfResultset = [];
            types.forEach((e, i) => {
                selfResultset.push([
                    e.filter(ee => e != null).join("~"),
                    counter[i][0]
                ]);
            });
            let selfArrObj = {};
            //遍历生成的type,合成对应的数据
            for (let j = types[0].length; j > 0; j--) {
