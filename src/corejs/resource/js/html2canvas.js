...
var NodeContainer = function () {
    function NodeContainer(node, parent, resourceLoader, index) {
        ...
        var _selfTransform = style.transform;
        this.style = {
            ...
            transform: (0, _transform.parseTransform)(style,node),
        ...
        if (node.tagName === 'IMG') {
            node.addEventListener('load', function () {
                ...
                if(node.nodeName === 'IMG' && /leaflet-tile/.test(node.className)){
                    var leftTop = _selfTransform.match(/-?\d+/g).slice(4,6);
                    _this.bounds.top += +leftTop[1];
                    _this.bounds.left += +leftTop[0];
                }
        ...
        if(node.nodeName === 'IMG' && /leaflet-tile/.test(node.className)){
            var leftTop = _selfTransform.match(/-?\d+/g).slice(4,6);
            this.bounds.top += +leftTop[1];
            this.bounds.left += +leftTop[0];
        }
...
var transformOrigin = exports.parseTransform = function parseTransform(style,node) {
    ...
    // node.nodeName !== 'IMG' && (transform = [1, 0, 0, 1, 0, 0]);
